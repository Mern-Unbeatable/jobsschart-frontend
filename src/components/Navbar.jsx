...
const navLinkClass = ({ isActive }) =>
  [
    "group relative inline-flex items-center whitespace-nowrap px-1.5 2xl:px-2 py-1 text-sm 2xl:text-base font-medium transition-all duration-300 ease-out",
    ...
    <nav className="sticky top-0 z-100 border-b border-gray-100 bg-[#E9D5FF] shadow-sm">
      <div className="relative z-10 w-full container mx-auto px-4 lg:px-6 ">
        <div className="flex h-16 md:h-20 items-center justify-between gap-3">
          ...
          <div className="hidden 2xl:flex min-w-0 flex-1 items-center justify-center gap-3 2xl:gap-4">
            ...
            >
              <button
                ...
                className={`group relative inline-flex items-center gap-1 whitespace-nowrap px-1.5 2xl:px-2 py-1 text-sm 2xl:text-base font-medium transition-all duration-300 ease-out hover:-translate-y-0.5 active:translate-y-0 cursor-pointer after:absolute after:left-0 after:-bottom-0.5 after:h-0.5 after:w-full after:origin-left after:rounded-full after:transition-transform after:duration-300 after:content-[''] ${
                  isEntrepreneurshipActive
                    ? "text-purple-700 after:scale-x-100 after:bg-purple-700"
          ...
          <div className="hidden 2xl:flex shrink-0 items-center gap-2 2xl:gap-3">
            <LanguageSelector />
            {isAuthenticated ? (
              <>
                <Link
                  to={getDashboardRoute(userRole)}
                  className="rounded-md bg-[#6E35AE] px-5 py-2 text-sm font-semibold text-white shadow-sm transition-all uppercase tracking-wide"
                >
                  Dashboard
                </Link>
              </>
              ...
              <>
                <Link
                  to={ROUTES.REGISTER}
                  className="rounded-md bg-white border border-gray-200 px-4 py-2 text-sm font-semibold text-[#6E35AE] shadow-sm transition-all hover:bg-gray-50 whitespace-nowrap"
                >
                  {t("common.becomeConsultant")}
                </Link>
                <Link
                  to={ROUTES.LOGIN}
                  className="rounded-md bg-[#6E35AE] px-5 py-2 text-sm font-semibold text-white shadow-sm transition-all whitespace-nowrap"
                >
                  {t("common.signIn")}
                </Link>
              </>
          ...
          <div className="flex 2xl:hidden items-center gap-2 sm:gap-3 ml-auto shrink-0">
            <LanguageSelector />
            <button
              type="button"
      ...
      {menuOpen && (
        <div
          className="fixed inset-0 top-16 md:top-20 z-90 bg-black/30 backdrop-blur-sm 2xl:hidden"
          onClick={closeMenu}
        >
          <div
            id="mobile-menu"
            className="absolute left-0 top-0 z-100 flex w-full flex-col border-t border-gray-100 bg-[#E9D5FF] shadow-2xl 2xl:hidden animate-in slide-in-from-top duration-200"

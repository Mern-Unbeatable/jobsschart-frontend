...
const TABLE_COLS = [
  ...
  { label: "Donator Type", width: "flex-1" },
...
const AdminDonation = () => {
  ...
  const { data: donationsData, isLoading: isDonationsLoading } =
    useGetDonationsQuery({ page, limit: PAGE_SIZE });
  ...
  const totalPages =
    ...
    Math.max(1, Math.ceil((donationsData?.meta?.total || 0) / PAGE_SIZE));
  ...
  const handleDelete = useCallback(
    async (id) => {
      try {
        ...
        toast.error(
          err?.data?.message || err?.message || "Failed to delete donation",
        );
      }
    },
    [deleteDonation],
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-green-500/60"></div>
      </div>
    );
  }
